...
### Tasks
- [ ] `GET /jobs` — list jobs visible to the current user (Admin: all; User: attached).
- [ ] `POST /jobs` — create job (Admin only). On creation, seed default stages from req 7.2 in a transaction.
- [ ] `GET /jobs/[id]` — job detail page (access guarded by `requireJobAccess`).
- [ ] `PATCH /jobs/[id]` — edit job title/description/status (Admin only).
- [ ] `POST /jobs/[id]/close` — close job (Admin only); data preserved.
- [ ] Jobs list UI with status badges and filters.
- [ ] Job create/edit dialog form.
- [ ] Job detail layout with tabs (Board, Candidates, Stages, Users, Reports).
- [ ] Audit log on job create/edit/close.
